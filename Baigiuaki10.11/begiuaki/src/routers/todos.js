...
router.put('/:id', (req, res) => {
    ...
    const validDueDate = due_date ? new Date(due_date).toISOString().slice(0, 19).replace('T', ' ') : null;

    db.query('UPDATE todos SET title = ?, description = ?, due_date = ?, completed = ? WHERE id = ?', 
    [title, description, validDueDate, completed, id], (err, result) => {
        if (err) return res.status(500).send(err);
        res.json({ id, title, description, due_date: validDueDate, completed });
    });
});
